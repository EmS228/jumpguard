...
system("pkill -9 libcamera-jpeg"); // Ensure no other libcamera-jpeg processes are running
    ...
    for (int cycle = 1; cycle<=num_cycles; ++cycle){
        ...
        for (int i = 1; i<=num_images; ++i){
            char command[200];
            snprintf(command, sizeof(command), "libcamera-still -o imageTest12/imageSet_%02d.png --encoding png --width 4608 --height 2592 -t 0 --nopreview -n --immediate --quality 10 2>&1", (i + (cycle-1)*10));
