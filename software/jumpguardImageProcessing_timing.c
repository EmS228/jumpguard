...
int main(){
    ...
    while(1){
        ...
        t = clock() - t;
        double elapsed_time = ((double)(t) / CLOCKS_PER_SEC); //convert to seconds
        printf("Elapsed time for processing: %.2f seconds\n", elapsed_time);
    }
...
int startup() {
    ...
    int diffValues[3] = {0}; // Array to store differences between images
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < width * height; j++) {
            diffValues[i] += abs(grayImage1[j] - grayImage2[j]);
        }
    ...
    printf("Difference values for the three images: %d, %d, %d\n", diffValues[0], diffValues[1], diffValues[2]);

    // Calculate the average difference and set the threshold
    int avgDiff = (diffValues[0] + diffValues[1] + diffValues[2]) / 3;
